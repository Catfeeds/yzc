...
class PassportController extends Controller
{
            ...
            if(isset($usersInfo[0])){//查到数据
                if (empty($usersInfo[0]->tel)) {
                   return msg_err(self::BIND_CODE,self::BIND_TEL,request('openid'));//绑定手机号
                    ...
                    $user = $usersObj::find($usersInfo[0]->id);
                ...
                $usersObj->openid = request('openid');
                $bool = $usersObj->save();
                if($bool == 1) {
                    return msg_ok(self::BIND_CODE,self::BIND_TEL,request('openid'));
                } else {
                    return msg_err(self::NO_STATUS,self::LOGIN_TREM);
                }
            }
    ...
    public function bindTel()
    {
        ...
        if (empty($info) ||
            ...
            !array_key_exists('openid',$info)) {//验证数据
            return msg_err(self::NO_STATUS,self::PARAM);
        }
        //验证码是否正确
        ...
        $bool = $usersObj->up_bind($info);
        if ($bool == self::BIND_STATUS_YES) {//成功
            $userInfo = $usersObj->sel_bind($info['openid']);
            $user = $usersObj::find($userInfo[0]->id);
            $user->api_token = $token = str_random(60);
            $user->save();
...
//        die;
        ...
        $newName = $baseName.'.'.$file->getClientOriginalExtension();$saveDir = 'upload/'.date('Y').'/'.date('m').'/'.date('d').'/'; $savePath = $saveDir.$newName;
