...
class CommentController extends Controller
{
    ...
    public function oneComment()
    {
        ...
        if (empty($commentInfo) ||
            !array_key_exists('forum_id',$commentInfo) ||
            !array_key_exists('api_token',$commentInfo)||
            !array_key_exists('content',$commentInfo)
        ) {
        ...
        if ($one_comments_id) {
            $forumM = new Forum();
            $forumBool = $forumM->replies_inc($commentInfo['forum_id']);
            if($forumBool)
            {
    ...
    public function twoComment()
    {
        $commentInfo = request()->all();
        if (empty($commentInfo) ||
            !array_key_exists('parent_id',$commentInfo) ||
            !array_key_exists('api_token',$commentInfo)||
            !array_key_exists('content',$commentInfo)||
            !array_key_exists('forum_id',$commentInfo)||
            !array_key_exists('one_users_id',$commentInfo)//一级评论发布者id
        ) {
            return msg_err(PassportController::NO_STATUS,PassportController::PARAM);
        }
        $redis = GetAppToken($commentInfo['api_token']);
    ...
    public function showComment($commentInfo=false)
    {
        ...
        if(isset($commentInfo['api_token'])&&$commentInfo['api_token']!=''){
            $redis = GetAppToken($commentInfo['api_token']);
            if($redis['code'] == 200){
                $commentInfo['users_id'] = $redis['u_id'];
                unset($commentInfo['api_token']);
    ...
    public function setFabulou()
    {
        ...
        if (empty($fabulouInfo) ||
            !array_key_exists('api_token',$fabulouInfo)||
            !array_key_exists('one_comment_id',$fabulouInfo)||
            !array_key_exists('yse_no_fabulou',$fabulouInfo)//0取消，1点赞
        ) {
            return msg_err(PassportController::NO_STATUS,PassportController::PARAM);
        }
...
//        $sumrepliesInfo = request()->all();
    ...
    public function twoPageComment()
    {
        ...
        if (empty($twoCommentInfo) ||
            !array_key_exists('parent_id',$twoCommentInfo)
        ) {
            return msg_err(PassportController::NO_STATUS,PassportController::PARAM);
        }
