...
class ReleaseController extends Controller
{
    ...
    public function releaseForum()
    {
        ...
        if (empty($forumInfo) ||
            !array_key_exists('title',$forumInfo) ||
            !array_key_exists('content',$forumInfo)||
            !array_key_exists('parent_type',$forumInfo)||
            !array_key_exists('type',$forumInfo)||
            !array_key_exists('photo',$forumInfo)||
            !array_key_exists('room_city',$forumInfo)
        ) {
    ...
    public function delForum()
    {
       $forumObj = new Forum();
        $data = request()->all();
        if(empty($data) || !array_key_exists('id',$data) || !array_key_exists('api_token',$data)){
            return msg_err(PassportController::NO_STATUS,PassportController::PARAM);
        }
        //从token中获取uid
        $redis = GetAppToken($data['api_token']);
