...
class ForumController extends Controller
{
    ...
    public function forumList()
    {
        ...
        if (isset($data['parent_type'])&&isset($data['id'])&&isset($data['type'])){//详情
            ...
            if(!isset($data['room_city']))
            {
                $data['room_city'] = 110000;
            }
    ...
    */
    ...
    public function collection()
    {
        ...
        if(empty($data)||
            ...
            !array_key_exists('parent_type',$data)||
            !array_key_exists('type',$data)||
            !array_key_exists('f',$data)//标示，是收藏还是取消收藏0:取消 1：收藏
        ){
    ...
    public function fabulous()
    {
        ...
        if(empty($data)||
            ...
            !array_key_exists('parent_type',$data)||
            !array_key_exists('type',$data)||
            !array_key_exists('f',$data)||//标示，是点赞还是取消点赞0:取消 1：收藏
            !array_key_exists('fabulous_users_yse_id',$data)//被点赞人id
        ){
            return msg_err(PassportController::NO_STATUS,PassportController::PARAM);
        }
        $redis = GetAppToken($data['api_token']);
    ...
    public function randomForum()
    {
        ...
        //查论坛表中共有多少数据
        $daRe = request()->all();
        if(!isset($daRe['room_city']))
        {
            $daRe['room_city'] = 110000;
        }
        $data = $forumM->f_count($daRe['room_city']);
