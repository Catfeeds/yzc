    ...
    public function insertHistory($data=false)
    {
        ...
        if(!isset($data['users_id']))//没有token---history_no
        {
            ...
            if($b){
                ...
                $bool1 =$obj->insert(['key'=>$data['key'],'values'=>serialize($data['values']),'num'=>self::ET,'status'=>$data['status'],'users_id'=>$data['users_id'],'updated_at'=>date('Y-m-d H:i:s')]);
            }
            $redis = new Redis();
